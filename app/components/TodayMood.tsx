...
/** The caregiver's glance at how Meimei says she is today. The face is the
 *  point, so it gets the middle and the room the button used to take; the way
 *  through to the rest is an arrow in the corner. */
export default function TodayMood() {
  const today = MOOD_ENTRIES[0];
  const mood = MOOD_BY_KEY[today.mood];

  const note = today.note
    ? today.note.length > PREVIEW
      ? `${today.note.slice(0, PREVIEW).trimEnd()}…`
      : today.note
    : null;
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-1 flex-col items-center justify-center py-2 text-center">
        <MoodFace mood={today.mood} className="h-24 w-24 xl:h-[116px] xl:w-[116px]" />
        <p className="mt-3 text-[17px] font-bold leading-6 tracking-tight text-neutral-900 xl:text-[18px]">
          {mood.label}
        </p>
        <p className="mt-0.5 text-[12px] leading-4 text-neutral-500">{today.when}</p>
      </div>
